...
window.EVAL_DATA = {
  ...
  recommendation: {
    ...
    summary: 'Most spec-faithful, fully type-safe, cleanest lint, complete test suite across all four tiers, security-hardened (explicit SameSite + live JWT refresh + isActive enforcement), canonical patterns throughout. No high- or medium-severity findings. The 2026-09-05 dynamic security review (filter confidence ≥ 8/10) corroborates the baseline: claude-5.2 and claude-5.3-flash were the only branches with no confirmed findings, while the worst confirmed defects elsewhere are an unauthenticated PUT /api/setup full-DB-restore takeover (opencode-5.1, pi-5.1) and a photo-route path traversal (opencode-5.1). The new #2, build-claude-glm-5.3-flash (composite 4.35, functional 100/100), is the strongest alternative baseline ever produced: matches claude-5.2 at the functional ceiling (25/25 E2E), the 9-integration-test completeness, and the core security posture, with a smaller footprint (285 KB vs 357 KB) — but its non-functional lint pipeline, passwordChangedAt-only live refresh, and missing isActive gate leave claude-5.2 ahead on 3 of 5 dimensions.'
  },

  // ---------- Dynamic security review (§5.5, 2026-09-05) ----------
  // Adversarial per-branch security review of all 11 build branches (incl. excluded
  // build-ibm-bob): sub-agent identification + parallel false-positive filtering;
  // only findings at filter confidence >= 8/10 are recorded. Review-only pass —
  // it does not alter the static stars, composites, or ranks (prior results stand).
  // One row per affected branch so per-branch pages can filter by exact branch name.
  securityReview: {
    date: '2026-09-05',
    confidenceThreshold: 8,
    method: 'Adversarial per-branch security review: sub-agent identification of high-confidence vulnerabilities per branch, then parallel independent false-positive filtering; findings kept at filter confidence >= 8/10. Read-only source analysis — no code executed, no branches patched.',
    summary: '13 branch-level findings confirmed across 8 of the 10 scored branches; build-ibm-bob (excluded from ranking) had 2 more; build-claude-glm-5.2 and build-claude-glm-5.3-flash had none. Most severe: unauthenticated PUT /api/setup full-DB-restore takeover (opencode-5.1, pi-5.1) and photo-route path traversal (opencode-5.1).',
    findings: [
      { sev: 'High', branch: 'build-opencode-glm-5.1', finding: 'Unauthenticated PUT /api/setup full-DB-restore takeover: the PUT handler accepts a full backup JSON payload with no auth and no setup-lock check; an all-empty-arrays payload passes schema validation, wipes every user/item/sale row, and re-opens setup so the attacker creates their own admin.', location: 'src/app/api/setup/route.ts:84', filterConf: 9 },
      { sev: 'High', branch: 'build-pi-glm-5.1', finding: 'Unauthenticated PUT /api/setup full-DB-restore takeover: same pattern as opencode-5.1 — unauthenticated PUT accepts a backup payload; all-empty-arrays restore wipes the DB and re-opens setup for attacker admin creation.', location: 'src/app/api/setup/route.ts:50', filterConf: 9 },
      { sev: 'High', branch: 'build-opencode-glm-5.1', finding: 'Photo-route path traversal: the filename path segment is URL-decoded before joining the uploads directory, so %2F/.. sequences survive the extension check — GET /api/photos/<id>/..%2F..%2Fdata%2Fsqlite.db reads the SQLite database outside /data/uploads (bcrypt hashes, full inventory/sales data).', location: 'src/app/api/photos/[itemId]/[filename]/route.ts:23', filterConf: 8 },
      { sev: 'High', branch: 'build-ibm-bob', finding: 'Photo-route path traversal (same pattern as opencode-5.1): URL-decoded filename escapes the uploads directory via getPhotoPath and can read the SQLite database. (Branch excluded from scoring; recorded for completeness.)', location: 'src/app/api/photos/[itemId]/[filename]/route.ts:44-56; src/lib/utils.ts:67-69', filterConf: 8 },
      { sev: 'High', branch: 'build-opencode-minimax-m3', finding: 'BOLA in the sale-creation itemName branch: the item is matched by name with no ownership check, so any authenticated user can sell (and flip the status of) another user\'s item; GET /api/sales/[id] additionally embeds the full (possibly other-user\'s) item record in its response.', location: 'src/app/api/sales/route.ts:102; src/app/api/sales/[id]/route.ts', filterConf: 9 },
      { sev: 'Med', branch: 'build-claude-glm-5.1', finding: 'Session-revocation failure: withAuth trusts JWT role/canViewAll/isActive frozen at issuance — no per-request DB re-check and deactivation/demotion never bumps passwordChangedAt, so a deactivated or demoted account keeps full API access until the 30-day token expires.', location: 'src/lib/auth.ts jwt callback; src/lib/api-utils.ts withAuth', filterConf: 8 },
      { sev: 'Med', branch: 'build-opencode-glm-5.2', finding: 'Session-revocation failure: withAuth trusts JWT role/canViewAll/isActive frozen at issuance — no per-request DB re-check and deactivation/demotion never bumps passwordChangedAt, so a deactivated or demoted account keeps full API access until the 30-day token expires.', location: 'src/lib/auth.ts jwt callback; src/lib/api-utils.ts withAuth', filterConf: 8 },
      { sev: 'Med', branch: 'build-pi-glm-5.2', finding: 'Session-revocation failure: withAuth trusts JWT role/canViewAll/isActive frozen at issuance — no per-request DB re-check and deactivation/demotion never bumps passwordChangedAt, so a deactivated or demoted account keeps full API access until the 30-day token expires. (passwordChangedAt itself is correctly written at login — the pi-5.1 static bug is fixed here.)', location: 'src/lib/auth.ts jwt callback; src/lib/api-utils.ts withAuth', filterConf: 8 },
      { sev: 'Med', branch: 'build-vscode-glm-5.2', finding: 'Session-revocation failure: withAuth trusts JWT role/canViewAll/isActive frozen at issuance — no per-request DB re-check and deactivation/demotion never bumps passwordChangedAt, so a deactivated or demoted account keeps full API access until the 30-day token expires.', location: 'src/lib/auth.ts jwt callback; src/lib/api-utils.ts withAuth', filterConf: 8 },
      { sev: 'Med', branch: 'build-codex-glm-5.2', finding: 'Session-revocation failure: withAuth trusts JWT role/canViewAll/isActive frozen at issuance — no per-request DB re-check and deactivation/demotion never bumps passwordChangedAt, so a deactivated or demoted account keeps full API access until the 30-day token expires.', location: 'src/lib/auth.ts jwt callback; src/lib/api-utils.ts withAuth', filterConf: 8 },
      { sev: 'Med', branch: 'build-opencode-minimax-m3', finding: 'Session-revocation failure: withAuth trusts JWT role/canViewAll/isActive frozen at issuance — no per-request DB re-check and deactivation/demotion never bumps passwordChangedAt, so a deactivated or demoted account keeps full API access until the 30-day token expires. Variant: the isActive check code path is present but dead (unreachable).', location: 'src/lib/auth.ts jwt callback; src/lib/api-utils.ts withAuth', filterConf: 8 },
      { sev: 'Med', branch: 'build-pi-glm-5.1', finding: 'Session-revocation failure: withAuth trusts JWT role/canViewAll/isActive frozen at issuance and passwordChangedAt is never written to the JWT at all (the §5.2 static bug), so neither password changes nor deactivation/demotion invalidate existing sessions until the 30-day token expires.', location: 'src/lib/auth.ts jwt callback; src/lib/api-utils.ts withAuth', filterConf: 8 },
      { sev: 'Med', branch: 'build-ibm-bob', finding: 'Session-revocation failure: withAuth trusts JWT role/canViewAll/isActive frozen at issuance — no per-request DB re-check and deactivation/demotion never bumps passwordChangedAt, so a deactivated or demoted account keeps full API access until the 30-day token expires. (Branch excluded from scoring; recorded for completeness.)', location: 'src/lib/auth.ts jwt callback; src/lib/api-utils.ts withAuth', filterConf: 8 },
      { sev: 'Med', branch: 'build-vscode-glm-5.2', finding: 'Cross-tenant IDOR via POST /api/import (type=sales): sales rows are inserted for a caller-supplied itemId with no ownership check — a standard user can create sales against another user\'s items, corrupting that user\'s inventory status, derived stock, and financial reports.', location: 'src/app/api/import/route.ts:82-121', filterConf: 8 },
      { sev: 'Med', branch: 'build-opencode-glm-5.1', finding: 'Cross-tenant IDOR via POST /api/import (type=sales): sales rows are inserted for a caller-supplied itemId with no ownership check — a standard user can create sales against another user\'s items, corrupting that user\'s inventory status, derived stock, and financial reports.', location: 'src/app/api/import/route.ts:71', filterConf: 8 }
    ],
    belowBar: 'Candidates filtered out below the 8/10 bar (transparency, not counted as findings): CSV formula injection via spreadsheet import (conf 2-3), admin-only restore photos.path traversal (conf 3), seed.ts hardcoded admin credentials (conf 2-7), setup-unlock recovery design (conf 2-4).',
    cleanBranches: ['build-claude-glm-5.2', 'build-claude-glm-5.3-flash']
  },
  ...
  // functionalWinner: an object { branch, score, passFraction } per FUNCTIONAL_EVAL_PROMPT.md rev 3 —
