...
The newly added `build-opencode-minimax-m3` is a co-runner-up at 96/100 (tied with pi-5.2 and vscode-5.2), failing only REG-06 (login-only JWT refresh — the same shared gap). All 5 flows pass (7/7 branch e2e tests + 17/18 regression scenarios). The app boots correctly (`src/proxy.ts` exports `proxy` — the correct Next.js 16 name, unlike codex-5.2). Admin created via `/api/setup` fallback (`seed.ts` crashes with `Cannot find module 'dotenv/config'` — dotenv not in package.json). `AUTH_SECRET` env var required (NextAuth throws `MissingSecret` without it). First build generated by a non-GLM model (MiniMax M3).
...
**Agent:** opencode 1.18.3 · **Boot:** dev (AUTH_SECRET required) · **Admin:** admin@example.com (via /api/setup) · **Functional score:** 96/100 (24/25 tests pass)
...
**Recommendation:** `build-claude-glm-5.2` is the only build that passes the full functional E2E suite (26/26, 100/100). It is the only build with working session invalidation (REG-06), the only build where sale/inventory creation does not return 500, and the only build where admin user management (REG-11) works end-to-end. This corroborates the static-analysis ranking in BUILD_EVALUATION.md, which also ranked claude-5.2 #1. Adopt it as the production baseline. The `build-pi-glm-5.2` build is a co-runner-up alongside `build-vscode-glm-5.2` and `build-opencode-minimax-m3` (all 96/100, failing only REG-06) — all are strong candidates for a second-tier baseline, and pi-5.2 is the most-improved build in the cohort (+34 points over pi-5.1). The newly added `build-opencode-minimax-m3` is the first build generated by a non-GLM model (MiniMax M3) and matches the GLM-5.2 runner-ups functionally despite the different model lineage. Their only shared failure is the REG-06 session-invalidation gap, a single moderate-effort fix (add live JWT refresh of passwordChangedAt in the jwt callback). The `build-opencode-glm-5.2` build (88/100) is fifth; its broken seed.ts should be fixed before any adoption. The remaining three builds (opencode-5.1, pi-5.1, claude-5.1) all have a broken sale-creation endpoint that cascades to 5+ regression failures and require substantial remediation before they are functionally viable. The `build-codex-glm-5.2` is the only build that does not boot at all — a single boot-blocking bug (`src/proxy.ts` exports `middleware` instead of the Next.js 16 required `proxy`) causes HTTP 500 on every route, so its functional score is 0/100 (0/26). This is a trivial S-effort fix (rename one function), but it was not patched (per the no-patching fidelity rule), so the app never reached a booted state and no E2E tests could run. codex-5.2 enters at functional rank #8 (new last place), below pi-5.1 (62/100) and claude-5.1 (35/100).
...
|---|---|---|---|---|---|
| 2026-07-16 | `build-opencode-minimax-m3` (opencode 1.18.3 / MiniMax M3) | Run on opencode-minimax-m3 only (7 branch e2e tests + 18 regression scenarios × 3 retries, fresh DB + dev-server restart per run) | No — opencode-minimax-m3's score of 96/100 does not strictly exceed the current leader's 100/100; per the strict-`>` re-run rule, no existing branches were re-evaluated | 96/100 (24/25 pass; only REG-06 fails) | opencode-minimax-m3 enters at functional rank #2 (co-runner-up with pi-5.2 and vscode-5.2 at 96/100). No existing branch's rank changed (claude-5.2 remains #1). |
