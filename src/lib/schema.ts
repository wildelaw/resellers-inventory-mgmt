...
export const users = sqliteTable('users', {
  ...
  canViewAll: integer('can_view_all').notNull().default(0), // boolean: 0 or 1
  isActive: integer('is_active').notNull().default(1), // boolean: 0 or 1
  passwordChangedAt: integer('password_changed_at').notNull().default(0), // unix timestamp
  createdAt: integer('created_at').notNull().$defaultFn(() => Math.floor(Date.now() / 1000)),
  updatedAt: integer('updated_at').notNull().$defaultFn(() => Math.floor(Date.now() / 1000)),
  createdBy: integer('created_by'), // FK to users.id, managed in application logic
...
export const photos = sqliteTable('photos', {
  ...
  isPrimary: integer('is_primary').notNull().default(0), // boolean: 0 or 1
  createdAt: integer('created_at').notNull().$defaultFn(() => Math.floor(Date.now() / 1000)),
}, (table) => [
...
export const appConfig = sqliteTable('app_config', {
  ...
  setupComplete: integer('setup_complete').notNull().default(0), // boolean: 0 or 1
  updatedAt: integer('updated_at').notNull().$defaultFn(() => Math.floor(Date.now() / 1000)),
});
