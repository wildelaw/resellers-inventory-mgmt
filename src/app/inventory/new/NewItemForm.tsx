...
export default function NewItemForm() {
  ...
  const [form, setForm] = useState({
    name: '', description: '', purchaseDate: new Date().toISOString().split('T')[0],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) { const data = await res.json(); throw new Error(data.error || 'Failed to create item'); }
